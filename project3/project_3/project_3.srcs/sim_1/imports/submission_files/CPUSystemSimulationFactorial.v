...
module CPUSystemSimulationFactorial();
    ...
    initial begin
        ...
        //              STA     R2, 0x54 	# B4 0x7D54     # M[ADDRESS] â†? R2
        ...
        // END_F		RET                 # CA 0x6C00
        // MULTIPLY	    MOV     R3, R2      # CC 0x5B50     # for R2 â†? R2*R4
        //              DEC     R4, R4      # CE 0x23F0
        // M_LOOP	    ADD     R2, R2, R3  # D0 0x4EDC     # R4 times R2 â†? R2 + R3
        //              DEC     R4, R4      # D2 0x23F0
        ...
        while (test_no <= 69 && clock_count <= 1500) begin
            ...
            if (CPUSys.ALUSys.ARF.PC.Q >= 16'h00D0) begin
                            $display("Time: %t | Clk: %4d | T: %04x | PC: %04x | IROut: %04x | IMU_CS: %b",
                                     $time,
                                     clock_count, 
                                     CPUSys.T, 
                                     CPUSys.ALUSys.ARF.PC.Q, 
                                     CPUSys.ALUSys.IMU.IR.IROut,
                                     CPUSys.IMU_CS);
                                     end
