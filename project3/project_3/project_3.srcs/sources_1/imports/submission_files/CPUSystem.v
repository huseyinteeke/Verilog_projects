...
always @(*) begin
        ...
        case(T)
            ...
            12'h0004: begin 
                ...
                begin
                    ...
                    ARF_FunSel  = 2'b00;        // Ýþlemsiz
                    
                    RF_OutASel  = 3'b000;       // R1 seçili (veya Decoded_RSEL)
                    ALU_FunSel  = 4'b0000;      // Ýþlemsiz geç
                ...
                else if(Opcode == 6'd26) begin // CALL - T=4 (PC'yi Hazýrla)
                    // PC'yi 006F'den 0070'e artýr. Henüz baþka bir yere yazmýyoruz.
                    ARF_OutCSel = 2'b00;       // OutC = PC
                    MuxASel     = 2'b01;       // MUXA = OutC
                    
                    RF_ScrSel   = 4'b0111;     // Sadece S1 Seçili [cite: 54]
                    RF_FunSel   = 2'b01;       // LOAD
                    RF_RegSel   = 4'b1111;     // Diðer yazmaçlarý kilitliyoruz [cite: 52]
                    T_Reset     = 1'b0;
                end
            ...
            12'h0008: begin 
                ...
                begin
                    ...
                    RF_OutASel  = 3'b000;       // Veri yolunu DÜÞÜRME, tut!
                    ALU_FunSel  = 4'b0000;
                    MuxCSel     = 1'b1;
    ...
    // S1'den MSB'yi (00) oku, FD adresine yaz ve SP'yi düþür
            ...
            12'h0010: begin 
                ...
                begin
                    ARF_OutDSel = 1'b1;         // Adres = SP (Artýk FF)
                    ARF_RegSel  = 3'b111;       // SP KÝLÝTLÝ (Yazmayý kapat)
                    ARF_FunSel  = 2'b00;
                ...
                begin
                    ARF_OutDSel = 1'b1;         // Adres = SP (Artýk 0xFE)
                    ARF_RegSel  = 3'b111;       // SP Kilitli (Yazma yok)
                    ARF_FunSel  = 2'b00;
                    ...
                    // S1'den LSB'yi (70) oku, FC adresine yaz ve SP'yi düþür
            ...
            12'h0020: begin
                ...
                ARF_OutDSel = 1'b1;         
                ARF_RegSel  = 3'b111;       // SP KÝLÝTLÝ
                ARF_FunSel  = 2'b00;
                
                DMU_CS      = 1'b0;         // RAM ÝÞÝ BÝTTÝ, KAPAT
                DMU_WR      = 1'b0;
                
                // EÐER SENÝN TASARIMDA "Decoded_IMM_DST" KULLANILIYORSA ONU YAZ, 
                // AMA NORMALDE BURASI Decoded_RSEL OLMALIDIR.
                RF_RegSel   = Decoded_IMM_DST; // R2'yi Hedefle
                RF_FunSel   = 2'b01;        // RF LOAD (Yaz)
                MuxASel     = 2'b10;        // DMUOut'u (Data Register çýkýþýný) seç
                ...
                T_Reset     = 1'b1;
                    ...
                    // FÝNAL: IMUOut'taki 0036'yý PC'ye yükle ve bitir
                    ...
                    DMU_CS      = 1'b0;
                    DMU_WR      = 1'b0;
                    T_Reset     = 1'b1;        // SIFIRLA VE BAÞA DÖN!
                end
                ...
                begin
                    if(Decoded_SREG1 != 4'b1111) // RF'den RAM'e yazýlýyorsa
                    begin
                        RF_OutASel = {1'b0, SrcReg1[1:0]}; // R2 (veya hangi SREG1 seçiliyse) onu ver
                    end
                    else // ARF'den RAM'e yazýlýyorsa
                    begin
                        RF_OutASel = 3'b100; // O zaman S1'i kullan
                    end
            ...
            12'h0040: begin
                ...
                if(Opcode == 6'd26) begin // CALL - T=64 (FÝNAL ATLAYIÞ)
                    // IMUOut'taki hedef adresi (0036) MUX B üzerinden PC'ye yükle
                    MuxBSel     = 2'b11;    // MUX B çýkýþý = IMUOut (Tablo 9)
                    
                    ARF_RegSel  = 3'b011;   // PC Seçili
                    ARF_FunSel  = 2'b01;    // PC <- MUX B (LOAD)
                    
                    DMU_CS      = 1'b0;     // RAM Kapalý
                    DMU_WR      = 1'b0;
                    T_Reset     = 1'b1;     // ÝÞLEM BÝTTÝ, SIFIRLA!
                end
            ...
            12'h0100: begin
                if(Opcode == 6'd26) begin // CALL (T=256)
                    ...
                    ARF_RegSel  = 3'b011;  // PC Seçili
                    ARF_FunSel  = 2'b01;   // PC'ye ALU çýkýþýný Yükle (LOAD)
                    
                    T_Reset     = 1'b1;    // VE FÝNAL!
                end
